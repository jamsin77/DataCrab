...
- 处理大数据时必须周期性 `log("info", f"已处理 {i}/{total} 行")` 或 `print()` 输出进度，避免长时间无输出被判定卡死

### 进度输出规范（所有耗时操作通用）
- 每个耗时操作（llm_vision/llm_chat/extract_keyframes/write_table_data/query_table_data）前后都 `print` 进度
- 循环体内也要 print（如"正在处理第 3/10 条"）
- 格式: `[步骤号/总步骤] 描述...` → 完成时: `完成: 数量/结果`
- 耗时超过 30 秒的操作必须加中间进度 print（如批量写入每 100 条打一次）
