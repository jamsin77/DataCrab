...
def _validate_extraction_consistency(
    body_text: str,
    ...
    重点检查：
    1. 正文章节数：源文档检测到的章节标题编号 vs 提取规则覆盖的章节编号
    2. 表格行列数：还原后的行列数 vs 源文本层可见的数据行数
    3. 合并单元格：检测「孤立空值」或「连续空值」（合并单元格未回填信号）
    4. 子列对应：检测多级表头（父项-子项）的父项值是否回填到每个对应行

    参数:
      body_text: 当前文档的正文文本（非表格部分）
      all_entries: LLM 提取的正文规则（已合并）
      table_infos: 表格还原信息，每项 {"table_name", "headers", "rows", "source_content"}
    ...
    for line in (body_text or "").split("\n"):
        ...
        m = heading_re.match(s)
        if m:
            source_headings.append(m.group(1))

    extracted_headings: set = set()
    for e in all_entries:
        code = str(e.get("rule_code") or "").strip()
        if code and re.match(r"^\d+(?:\.\d+){0,3}$", code):
            extracted_headings.add(code)
        name = str(e.get("rule_name") or "").strip()
        nm = re.match(r"^(\d+(?:\.\d+){0,3})\s*[、.．\s]", name)
        if nm:
            extracted_headings.add(nm.group(1))

    if source_headings:
        source_set = set(source_headings)
        covered = set()
        for sh in source_set:
            if sh in extracted_headings:
                covered.add(sh)
                continue
            # 允许父章节覆盖子章节（4.1.1 归入 4.1）
            for eh in extracted_headings:
                if eh.startswith(sh + "."):
                    covered.add(sh)
                    break
        missing = source_set - covered
        if missing:
            issue_msg = (
                f"正文章节缺失：源文档检测到 {len(source_set)} 个章节标题，"
                f"提取结果覆盖 {len(covered)} 个，缺失 {len(missing)} 个章节: "
                f"{sorted(missing)[:8]}"
            )
            # 缺失率 > 50% 且缺失数量超过 2 个视为严重，否则警告
            if len(missing) > max(len(source_set) * 0.5, 2):
                issues.append(issue_msg)
            else:
                warnings.append(issue_msg)
        print(
            f"  [校验] 正文章节：源文档 {len(source_set)} 个 -> 覆盖 {len(covered)} 个，"
            f"缺失 {len(missing)} 个"
        )

    # ============ 2/3/4. 表格校验 ============
    for ti in table_infos:
        tname = ti.get("table_name") or "未命名表"
        headers = ti.get("headers") or []
        rows = ti.get("rows") or []
        src_content = ti.get("source_content") or ""
        if not headers or not rows:
            warnings.append(f"表格「{tname}」缺少表头或数据行，无法校验行列")
            continue

        ncols = len(headers)
        nrows = len(rows)

        # --- 2. 行列数校验：从源文本层统计可见数据行数 ---
        est_rows = 0
        if src_content.strip():
            ...
            for line in src_content.split("\n"):
                ...
                if not s:
                    continue
                is_title, _, _, _ = _parse_table_title(s)
                if is_title or s in ("(续)", "（续）"):
                    continue
                if len(s) >= 3 and re.search(r"\d", s):
                    src_data_lines.append(s)
            est_rows = len(src_data_lines)
            if est_rows > 0:
                diff = abs(nrows - est_rows)
                tolerance = max(int(est_rows * 0.4), 2)
                if diff > tolerance:
                    ratio_msg = f"还原 {nrows} 行 vs 源文本层约 {est_rows} 行"
                    if nrows < est_rows:
                        warnings.append(f"表格「{tname}」行数偏少：{ratio_msg}，可能存在丢行")
                    else:
                        warnings.append(f"表格「{tname}」行数偏多：{ratio_msg}，可能存在拆行")
        print(
            f"  [校验] 表格「{tname}」: 还原 {nrows} 行 × {ncols} 列"
            + (f"，源文本层约 {est_rows} 行" if est_rows > 0 else "")
        )

        # --- 3. 合并单元格校验：孤立空值 / 连续空值 ---
        merged_cell_warned = False
        for col_idx, h in enumerate(headers):
            col_values: List[str] = []
            for r in rows:
                cell = r[col_idx] if col_idx < len(r) else ""
                col_values.append(str(cell).strip() if cell else "")
            # 孤立空值：前后行都有值，中间行空 -> 合并单元格未回填
            for i in range(1, len(col_values) - 1):
                if col_values[i] == "" and col_values[i - 1] != "" and col_values[i + 1] != "":
                    warnings.append(
                        f"表格「{tname}」列「{h}」存在孤立空值（前后行有值、中间行为空），"
                        f"疑似合并单元格未回填"
                    )
                    merged_cell_warned = True
                    break
            if merged_cell_warned:
                break
            # 连续空值：前一行有值，后续连续空但后面又出现值
            for i in range(len(col_values) - 2):
                if col_values[i] != "" and col_values[i + 1] == "" and col_values[i + 2] == "":
                    if any(v != "" for v in col_values[i + 2:]):
                        warnings.append(
                            f"表格「{tname}」列「{h}」存在连续空值（前一行有值、后续连续空），"
                            f"疑似合并单元格未回填"
                        )
                        merged_cell_warned = True
                        break
        ...
        for col_idx, h in enumerate(headers):
            ...
            if non_empty and empty_count > 0:
                warnings.append(
                    f"表格「{tname}」子列「{h}」（父项「{parent}」）有 {empty_count}/{len(col_values)} 行空值，"
                    f"疑似父项值未回填"
                )
                sub_col_warned = True
                break
        # 子列对应校验后不再阻断后续列检查，但本表只报告一次即可
        _ = sub_col_warned

    # 汇总结果
    if issues:
        return {"consistent": False, "issues": issues, "warnings": warnings}
    return {"consistent": True, "issues": [], "warnings": warnings}
